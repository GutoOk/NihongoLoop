...
async function startServer() {
  ...
  app.post("/api/ai/trigger-batch-jobs", ipRateLimit, authenticateRequest, async (req: any, res: any) => {
    try {
      ...
      if (!targetId) {
        throw new Error("Parâmetro targetId ausente.");
      }

      console.log(`[Server] Triggering batch jobs processing for targetId: ${targetId} with concurrency: ${concurrencyLimit || 3}`);

      const finalConcurrency = concurrencyLimit ? Math.max(1, Math.min(10, Number(concurrencyLimit))) : 3;

      processBatchJobsForTarget(req.supabase, req.user.id, targetId, getAi, finalConcurrency)
        .then((result) => {
          console.log(`[Server] Asynchronous batch processing finish for targetId: ${targetId}`, result);
        })
        .catch((err) => {
          console.error(`[Server] Asynchronous batch processing error for targetId: ${targetId}:`, err);
        });

      res.json({ status: "processing_started", targetId });
    } catch (e: any) {
      console.error("[trigger-batch-jobs]", e);
