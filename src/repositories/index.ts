...
export class ProcessingRunRepository {
  ...
  static async appendLog(id: string, message: string, meta?: any): Promise<void> {
    if (!isSupabaseConfigured) return;
    const run = await this.getRun(id);
    if (!run) return;
    
    const logs = Array.isArray(run.log) ? run.log : [];
    logs.push({ time: new Date().toISOString(), message, meta });
...
export class SourceRepository {
  static async getAll(): Promise<Source[]> {
    if (allowAuthBypass) return defaultMockSources;
    if (!isSupabaseConfigured) return [];
  ...
  static async getById(id: string): Promise<Source | null> {
    if (allowAuthBypass) return defaultMockSources.find(s => s.id === id) || null;
    if (!isSupabaseConfigured) return null;
...
export class SentenceRepository {
  static async getById(id: string): Promise<Sentence | null> {
    if (allowAuthBypass) return defaultMockSentences.find(s => s.id === id) || null;
    if (!isSupabaseConfigured) return null;
  ...
  static async getByIds(ids: string[]): Promise<Sentence[]> {
    if (allowAuthBypass) return defaultMockSentences.filter(s => ids.includes(s.id));
    if (!isSupabaseConfigured) return [];
  ...
  static async getBySourceId(sourceId: string): Promise<Sentence[]> {
    if (allowAuthBypass) return defaultMockSentences.filter(s => s.source_id === sourceId);
    if (!isSupabaseConfigured) return [];
  ...
  static async getBySource(sourceId: string): Promise<Sentence[]> {
    return this.getBySourceId(sourceId);
  }
  static async getByJapanese(japanese: string): Promise<Sentence[]> {
    if (allowAuthBypass) return defaultMockSentences.filter(s => s.japanese === japanese);
    if (!isSupabaseConfigured) return [];
  ...
  static async getAll(): Promise<Sentence[]> {
    if (allowAuthBypass) return defaultMockSentences;
    if (!isSupabaseConfigured) return [];
  ...
  static async findProcessedByJapaneseKeys(keys: string[]): Promise<Sentence[]> {
    ...
    // Buscar frases que não estão com status raw, para garantir alguma hidratação
  ...
  static async addBatch(sentences: Omit<Sentence, 'id' | 'created_at' | 'updated_at'>[]): Promise<Sentence[]> {
    ...
    const enriched = sentences.map(s => {
      const copy = { ...s, user_id: s.user_id || getUserId() } as any;
      if (copy.tags && Array.isArray(copy.tags)) {
        copy.tags = copy.tags.filter((t: any) => typeof t === 'string' && t.trim() !== '');
      } else {
        delete copy.tags;
      }
      return copy;
...
export function deserializeSentenceTerm(term: any): SentenceTerm {
  return term;
}

export function serializeSentenceTermInput(term: any): any {
  return term;
}

export function deserializeDictionaryEntry(entry: any): DictionaryEntry {
  return entry;
}

export function serializeDictionaryEntryInput(entry: any): any {
  return entry;
}
...
export class TermRepository {
  static async getAll(): Promise<SentenceTerm[]> {
    ...
    try {
      ...
      return (data || []).map(deserializeSentenceTerm);
    } catch {
      return [];
    }
  }
  ...
  static async getBySentence(sentenceId: string): Promise<SentenceTerm[]> {
    if (allowAuthBypass) return defaultMockTerms.filter(t => t.sentence_id === sentenceId);
    if (!isSupabaseConfigured) return [];
    const { data: termsData } = await supabase!.from('sentence_terms').select('*').eq('sentence_id', sentenceId).eq('user_id', getUserId());
    const rawTerms = (termsData || []).map(deserializeSentenceTerm);
    
    if (rawTerms.length > 0) {
      const { data: sentence } = await supabase!.from('sentences').select('japanese').eq('id', sentenceId).maybeSingle();
      if (sentence && sentence.japanese) {
         for (const t of rawTerms) {
            const currentSub = (sentence.japanese || '').substring(t.start_index, t.end_index);
            if (currentSub !== t.surface) {
               let bestStart = -1;
               let minDiff = Infinity;
               let pos = sentence.japanese.indexOf(t.surface);
               while (pos !== -1) {
                  ...
                  pos = sentence.japanese.indexOf(t.surface, pos + 1);
               }
               
               if (bestStart !== -1) {
                  const updatedStart = bestStart;
                  const updatedEnd = bestStart + t.surface.length;
                  t.start_index = updatedStart;
                  t.end_index = updatedEnd;
               }
            }
         }
      }
   ...
   static async getBySentences(sentenceIds: string[]): Promise<SentenceTerm[]> {
    if (allowAuthBypass) return defaultMockTerms.filter(t => sentenceIds.includes(t.sentence_id));
    if (!isSupabaseConfigured) return [];
    if (sentenceIds.length === 0) return [];
    
    const chunks: string[][] = [];
    for (let i = 0; i < sentenceIds.length; i += 100) {
      chunks.push(sentenceIds.slice(i, i + 100));
    }
    
    let allTermsData: any[] = [];
    for (const chunk of chunks) {
    ...
    const rawTerms = allTermsData.map(deserializeSentenceTerm);
    ...
    if (rawTerms.length > 0) {
      ...
      for (const chunk of chunks) {
         ...
         if (error) {
            console.error(error);
            throw new Error(`Erro do Supabase ao carregar frases correlatadas aos termos: ${error.message}`);
         }
         if (data) allSentences = allSentences.concat(data);
      }
      
      if (allSentences.length > 0) {
         const sentsMap = new Map<string, string>();
         for (const s of allSentences) {
            sentsMap.set(s.id, s.japanese);
         }
         
         for (const t of rawTerms) {
            const japanese = sentsMap.get(t.sentence_id);
  ...
  static async addBatch(terms: Omit<SentenceTerm, 'id' | 'created_at' | 'updated_at'>[]): Promise<SentenceTerm[]> {
    ...
    // 1. Local deduplication to prevent database-level collisions within the same insertion batch
    const uniqueTermsMap = new Map<string, any>();
    for (const t of terms) {
       const uInput = { ...serializeSentenceTermInput(t), user_id: t.user_id || getUserId() };
       const key = `${uInput.sentence_id}_${uInput.surface}_${uInput.start_index}_${uInput.end_index}`;
       // If already exists, prefer keeping the element that has notes/details
       if (!uniqueTermsMap.has(key) || (!uniqueTermsMap.get(key).context_meaning && uInput.context_meaning)) {
          uniqueTermsMap.set(key, uInput);
       }
    }
    const enriched = Array.from(uniqueTermsMap.values());
    if (enriched.length === 0) return [];

    // 2. Query the database for existing terms on the same sentences to prevent duplicate insertion
    const sentenceIds = Array.from(new Set(enriched.map(t => t.sentence_id).filter(Boolean)));
    ...
    if (sentenceIds.length > 0) {
       ...
       if (!fetchError && existingData) {
          for (const e of existingData) {
             const key = `${e.sentence_id}_${e.surface}_${e.start_index}_${e.end_index}`;
             existingKeys.add(key);
          }
       }
    }

    // 3. Filter out any local terms that already exist in the DB
    ...
    const { data, error } = await supabase!.from('sentence_terms')
      .insert(finalTermsToInsert)
      .select();
  ...
  static async update(id: string, updates: Partial<SentenceTerm>): Promise<SentenceTerm | null> {
    if (!isSupabaseConfigured) return null;
    const serialized = serializeSentenceTermInput(updates);
    const { data, error } = await supabase!.from('sentence_terms').update(serialized).eq('id', id).eq('user_id', getUserId()).select().maybeSingle();
    if (error) {
      console.error(error);
      throw new Error(`Erro do Supabase ao atualizar termo: ${error.message}`);
    }
    return data ? deserializeSentenceTerm(data) : null;
  }
  static async delete(id: string): Promise<boolean> {
    if (!isSupabaseConfigured) return false;
...
export class DictionaryRepository {
  ...
  static async getByIds(ids: string[]): Promise<DictionaryEntry[]> {
    ...
    return allData.map(deserializeDictionaryEntry);
  }
  ...
  static async addBatch(entries: Omit<DictionaryEntry, 'id' | 'created_at' | 'updated_at'>[]): Promise<DictionaryEntry[]> {
    ...
    const enriched = entries.map(e => {
      const copy = { ...serializeDictionaryEntryInput(e), user_id: e.user_id || getUserId() } as any;
      if (copy.tags && Array.isArray(copy.tags)) {
        copy.tags = copy.tags.filter((t: any) => typeof t === 'string' && t.trim() !== '');
      } else {
        delete copy.tags;
    ...
    const { data, error } = await supabase!.from('dictionary_entries').insert(enriched).select();
    if (error) {
      console.error(error);
  ...
  static async update(id: string, updates: Partial<DictionaryEntry>): Promise<DictionaryEntry | null> {
    if (!isSupabaseConfigured) return null;
    const copy = { ...serializeDictionaryEntryInput(updates) } as any;
    if (copy.tags && Array.isArray(copy.tags)) {
      copy.tags = copy.tags.filter((t: any) => typeof t === 'string' && t.trim() !== '');
    } else {
      delete copy.tags;
...
export class ProgressRepository {
  ...
  static async updateSentenceProgressLog(sentenceId: string, isCorrect: boolean): Promise<SentenceProgress | null> {
    ...
    const now = new Date();
    if (!existing) {
      const interval = isCorrect ? 10 : 1; // 10 minutes vs 1 minute
      const dueAt = new Date(now.getTime() + interval * 60000);
      return this.upsertSentenceProgress({
         sentence_id: sentenceId, user_id: getUserId(),
         seen_count: 1, correct_count: isCorrect ? 1 : 0, wrong_count: isCorrect ? 0 : 1,
      ...
      const dueAt = new Date(now.getTime() + interval * 60000);
      const mastery = isCorrect ? Math.min(100, (existing.mastery || 0) + 10) : Math.max(0, (existing.mastery || 0) - 15);
  ...
  static async updateDictionaryProgressLog(dictionaryEntryId: string, isCorrect: boolean): Promise<DictionaryProgress | null> {
    ...
    const now = new Date();
    if (!existing) {
      const interval = isCorrect ? 10 : 1;
      const dueAt = new Date(now.getTime() + interval * 60000);
      return this.upsertDictionaryProgress({
         dictionary_entry_id: dictionaryEntryId, user_id: getUserId(),
         seen_count: 1, correct_count: isCorrect ? 1 : 0, wrong_count: isCorrect ? 0 : 1,
         last_seen_at: now.toISOString(), 
         mastery: isCorrect ? 10 : 0,
         srs_interval_minutes: interval,
  ...
  static async applyFlashcardFeedback(dictionaryEntryId: string, feedback: 'again' | 'hard' | 'good' | 'easy'): Promise<DictionaryProgress | null> {
    ...
    if (feedback === 'again') {
       ...
       correctCount++;
       if (interval <= 0) {
          interval = 10; // 10 minutes for first correct card
       } else if (interval === 10) {
    ...
    if (existing) {
       return this.upsertDictionaryProgress({ id: existing.id, ...payload });
    } else {
       return this.upsertDictionaryProgress(payload);
    }
  }
}
...
export class AiJobRepository {
  ...
  static async claimJob(id: string, runnerId: string): Promise<boolean> {
     if (!isSupabaseConfigured) return false;
     const now = new Date();
     const lockedUntil = new Date(now.getTime() + 5 * 60000); // 5 min lock lease
  ...
  static async resetFailedJobs(): Promise<boolean> {
    ...
    const { error } = await supabase!.from('ai_jobs')
      ...
      .eq('user_id', getUserId())
      .in('status', ['error', 'running']); // Also reset running just in case
    return !error;
  }
...
export class StudySessionRepository {
  static async saveSession(session: Omit<StudySession, 'id' | 'created_at' | 'updated_at'>): Promise<StudySession | null> {
     if (!isSupabaseConfigured) return null;
     const { data, error } = await supabase!.from('study_sessions').insert(session).select().maybeSingle();
     if (error) console.error(error);
     return data;
  }
  ...
  static async getSourceOffset(sourceId: string): Promise<number> {
    ...
    try {
      ...
      return (data.config as any)?.offset || 0;
    } catch {
      return 0;
    }
  }
