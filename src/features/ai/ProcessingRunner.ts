...
export class ProcessingRunner {
  ...
  static async startPreparation(
    ...
    if (!run) {
      run = await ProcessingRunRepository.createRun(
        sourceId,
        options.runMode || "all",
      );
      if (run) {
    ...
    try {
      while (
          ...
          await ProcessingRunRepository.updateRun(runId, {
            ...
            log: [
              ...(run.log || []),
              {
                ...
                message: "Preparação cancelada pelo usuário.",
              },
            ],
          });
          break;
        ...
        const preparationStats =
          await SourcePreparationRepository.getStats(sourceId);
        ...
        if (pendingJobs.length === 0 && runningJobs.length === 0) {
          if (
            ...
            (runMode === "all" || runMode === "translate")
          ) {
            await ProcessingRunRepository.appendLog(
              runId,
              "Iniciando fase incremental de tradução...",
            );
            await SourcePreparationService.prepareSource(
              sourceId,
              { ...options, runMode: "translate" },
              runId,
            );
          } else if (
            hasMissingAnalysis &&
            (runMode === "all" || runMode === "analyze")
          ) {
            await ProcessingRunRepository.appendLog(
              runId,
              "Iniciando fase incremental de análise estrutural...",
            );
            await SourcePreparationService.prepareSource(
              sourceId,
              { ...options, runMode: "analyze" },
              runId,
            );
          } else if (
            hasMissingEnrichment &&
            (runMode === "all" || runMode === "dictionary")
          ) {
            await ProcessingRunRepository.appendLog(
              runId,
              "Iniciando fase incremental de enriquecimento de dicionário...",
            );
            await SourcePreparationService.prepareSource(
              sourceId,
              { ...options, runMode: "dictionary" },
              runId,
            );
          } else {
        ...
        const batchTranslate =
          runMode === "all" || runMode === "translate"
            ? pendingJobs.filter((j) => j.type === "batch_translate_sentences")
            : [];
        const batchAnalyze =
          runMode === "all" || runMode === "analyze"
            ? pendingJobs.filter((j) => j.type === "batch_analyze_sentences")
            : [];
        const batchDict =
          runMode === "all" || runMode === "dictionary"
            ? pendingJobs.filter((j) =>
                j.type.startsWith("batch_enrich_dictionary"),
              )
            : [];
        ...
        await ProcessingRunRepository.updateRun(runId, {
          created_jobs:
            pendingJobs.length + runningJobs.length + (run.processed_jobs || 0),
        });
        ...
        if (nextJobs.length > 0) {
          ...
          const useServerSide = (options.processMode || "server") === "server";

          if (useServerSide && isMigratedBatch) {
            const currentStepName = currentStepType.split("_")[1];
            await ProcessingRunRepository.updateRun(runId, {
              current_step: `Processando lotes de '${currentStepName}' em background no servidor (limite de ${concurrencyLimit} ativos)...`,
            });
            if (onProgress) onProgress();
            try {
              await AiJobService.triggerBatchJobs(
                sourceId,
                concurrencyLimit,
                abortController.signal,
              );
              const completedCount = targetJobs.filter(
                (j) =>
                  j.type === currentStepType &&
                  (j.status === "completed" ||
                    j.status === "error" ||
                    j.status === "applied"),
              ).length;
              ...
              if (err.name !== "AbortError") {
                await ProcessingRunRepository.appendLog(
                  runId,
                  `Erro ao delegar job de lote ao servidor: ${err.message}`,
                );
              }
            }
          ...
          const localStepName = currentStepType.split("_")[1];
          await ProcessingRunRepository.updateRun(runId, {
            current_step: `Processando lotes de '${localStepName}' localmente (limite de ${concurrencyLimit} ativos)...`,
          });
          if (onProgress) onProgress();
          ...
          try {
            ...
            for (const job of nextJobs) {
              ...
              const hasClaimed = await AiJobRepository.claimJob(
                job.id,
                this.runnerId,
              );
              if (!hasClaimed) {
              ...
              const jobPromise = (async () => {
                ...
                try {
                  const result = await AiJobService.processJobsBatch(
                    [job],
                    abortController.signal,
                  );
                  if (
                    ...
                    await ProcessingRunRepository.appendLog(
                      runId,
                      `Lote concluído: ${job.type.substring(6)} (${job.id.substring(0, 8)})`,
                    );

                    // Update processed_jobs real-time
                    const freshRun =
                      await ProcessingRunRepository.getRun(runId);
                    if (freshRun) {
                      await ProcessingRunRepository.updateRun(runId, {
                        processed_jobs: (freshRun.processed_jobs || 0) + 1,
                      });
                    }
                  } else {
                    ...
                    await ProcessingRunRepository.appendLog(
                      runId,
                      `Lote falhou: ${job.type.substring(6)} (${job.id.substring(0, 8)}) - ${errMsg}`,
                    );
                  }
                } catch (err: any) {
                  if (err.name !== "AbortError") {
                    await ProcessingRunRepository.appendLog(
                      runId,
                      `Erro ao processar lote: ${job.type.substring(6)} (${job.id.substring(0, 8)}) - ${err.message}`,
                    );
                  }
                } finally {
            ...
            if (err.name !== "AbortError") {
              await ProcessingRunRepository.appendLog(
                runId,
                `Erro geral durante o processamento: ${err.message}`,
              );
            }
          }
          ...
          await ProcessingRunRepository.updateRun(runId, {
            current_step: `Aguardando ${runningJobs.length} tarefas de IA em andamento...`,
          });
          if (onProgress) onProgress();
          await new Promise((r) => setTimeout(r, 2000));
        } else {
