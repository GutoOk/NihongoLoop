...
vi.mock('../../repositories', () => ({
  ...
  DictionaryFormRepository: {
    resolveOrCreate: vi.fn().mockResolvedValue({ id: 'form-1' })
  },
  DictionarySenseRepository: {
    resolveOrCreate: vi.fn().mockResolvedValue({ id: 'sense-1' })
  }
}));
  ...
  describe('detectWordsInSentences', () => {
    ...
    it('does not create duplicate dictionary entries if the word already exists', async () => {
      ...
      expect(TermRepository.addBatch).toHaveBeenCalledWith(expect.arrayContaining([
        expect.objectContaining({ dictionary_form_id: 'form-1', surface: expect.any(String) })
      ]));
    });
